...
def generate_otp_service(db: Session, device_id: int, user: User) -> dict:
    ...
    # Rate limit: check if there's an unexpired, unused OTP for this device+user
    existing_otp = (
        db.query(OTPCode)
        .filter(
    ...
    if existing_otp:
        email_sent = send_otp_email(user.email, existing_otp.code, device.name)
        return {
            "otp_id": existing_otp.id,
            "code": existing_otp.code,
            "expires_at": existing_otp.expires_at,
            "message": f"An active OTP already exists. Valid until {existing_otp.expires_at.strftime('%H:%M:%S')}.",
    ...
    return {
        ...
        "code": code,
        "expires_at": expires_at,
        "message": f"OTP generated for device {device.name}. Valid for 5 minutes.",
        "email_sent": send_otp_email(user.email, code, device.name),
    }
